...
export function useCreateAppointment() {
  ...
  return useMutation({
    mutationFn: async (payload: { doctorId: string; date: string; startTime: string; type?: string; notes?: string }) =>
      (await api.post("/appointments", payload)).data.data,
...
export function useCancelAppointment() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (id: string) => (await api.delete(`/appointments/${id}`)).data.data,
    onSuccess: () => qc.invalidateQueries({ queryKey: ["appointments"] }),
  });
}
